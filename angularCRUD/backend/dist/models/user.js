...
const create = (user, callback) => {
    ...
    console.log(user);
    db_1.db.query(queryString, [user.nume, user.prenume, user.email, user.datanastere, user.telefon,
        user.poza, user.cnp], (err, result) => {
        if (err) {
            callback(err);
        }
...
const update = (user, callback) => {
    ...
    db_1.db.query(queryString, [user.nume, user.prenume, user.email, user.telefon, user.poza, user.cnp, user.datanastere, user.id], (err, result) => {
        if (err) {
            callback(err);
        }
